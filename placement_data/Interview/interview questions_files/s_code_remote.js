// Get the hierarchy value from the KeyLime variable window.epulse_content_group
var hier1=""
function getHierName (cgValue) {

	if (window.epulse_content_group) {
	
        	var queryParts = window.epulse_content_group.split(",");
        	for (i=0; i < queryParts.length; i++) {
	
        	        if (queryParts[i].indexOf(cgValue) != -1) {
				
        	                var segment = queryParts[i];
        	                var value = segment.split("=");
        	                return value[1];
        	        }
        	}
        	return "";
...
if (window.epulse_content_group) {
	...
	var cg1 = getHierName("cg1");
	var cg2 = getHierName("cg2");
	var cg3 = getHierName("cg3");
...
/*var s_pageName=""*/
var s_pageName=(!s_pageName)?"":(s_pageName);
var s_server=""
var s_channel=""
var s_pageType=(!s_pageType)?"":(s_pageType);
/*var s_prop1=""*/
var s_prop1=(!s_prop1)?"":(s_prop1);
var s_prop2 = window.location.hostname;
var s_prop3=(!s_prop3)?"":(s_prop3);

/* persistent properties*/
var s_prop4=(!s_prop4)?"":(s_prop4);
var s_prop5=(!s_prop5)?"":(s_prop5);
var s_prop6=(!s_prop6)?"":(s_prop6);
var s_prop7=(!s_prop7)?"":(s_prop7);
var s_prop8=(!s_prop8)?"":(s_prop8);

/* more */
var s_prop9=(!s_prop9)?"":(s_prop9);
var s_prop10=(!s_prop10)?"":(s_prop10.toLowerCase());

/* campaign path */
var a = location.search.substring(1);
var b = (!a.match("campaign_id="))?"":(a.split("campaign_id="));
var c = (!s_pageName)?(location.hostname + location.pathname):(s_pageName);
var s_prop14=(!b)?"":(b[1] + " : " + c);

/* more */
var s_prop15=(!s_prop15)?"":(s_prop15.toLowerCase());
var s_prop16=(!s_prop16)?"":(s_prop16.toLowerCase());
var s_prop25=(!s_prop25)?"":(s_prop25.toLowerCase());


//Grabs the referring Subdomain
var dr=document.referrer;
var dstart=dr.indexOf('//')>-1?dr.indexOf('//')+2:0;
var dend=dr.indexOf('/',dstart)?dr.indexOf('/',dstart):dr.length;
var refsubdomain=dr.substring(dstart,dend);
var s_linkInternalFilters="javascript:,businessweek.com,dev16";
//Populate an sprop only if the referring subdomain is not in the internal url filters list
var s_prop11=s_linkInternalFilters.indexOf(refsubdomain)==-1?refsubdomain:"";
/* E-commerce Variables */
var s_campaign=(!s_campaign)?"":(s_campaign);
var s_state=""
var s_zip=""
var s_events=(!s_events)?"":(s_events.toLowerCase());
var s_products=(!s_products)?"":(s_products.toLowerCase());
var s_purchaseID=(!s_purchaseID)?"":(s_purchaseID);

/*var s_eVar1=""*/
var s_eVar1=(!s_eVar1)?"":(s_eVar1);
var s_eVar2=(!s_eVar2)?"":(s_prop16);
var s_eVar3=(!s_eVar3)?"":(s_eVar3.toLowerCase());
var s_eVar4=(!s_eVar4)?"":(s_eVar4.toLowerCase());
var s_eVar5=(!s_eVar5)?"":(s_eVar5.toLowerCase());
var s_eVar6=(!s_eVar6)?"":(s_eVar6.toLowerCase());
var s_eVar7=s_pageName;
var s_eVar8=(!s_eVar8)?"":(s_eVar8);
var s_eVar9=(!s_eVar9)?"":(s_eVar9);
...
var s_account="businessweekpoc";
var s_fpcdomain="businessweek.com";
var s_fpcsubdomain="om";
var s_sslfpcsubdomain="oms";
var s_vmk="43D8135C";
/* E-commerce Config */
...
/*var s_linkInternalFilters="javascript:,businessweek.com"*/
...
function s_doPlugins() {
	...
	s_vp_getCGI('s_campaign','campaign_id');
	
	s_vp_getCGI('s_eVar1','link_position');
	
	s_vp_getCGI('s_eVar3','campaign_medium');
	
	s_vp_getCGI('s_eVar4','campaign_type');
	
	s_vp_getCGI('s_eVar5','campaign_bucket');
	
	s_vp_getCGI('s_eVar6','campaign_creative');
	
	s_vp_getCGI('s_eVar8','chan');
	
	s_vp_getCGI('s_eVar9','sub');
	
	s_vp_setValOnce('s_campaign');
	
	s_vp_setAndPersistValue('s_prop8',s_vp_getValue('s_campaign'),0);

	s_vp_setAndPersistValue('s_prop7',s_vp_getValue('s_eVar3'),0);

	s_vp_setAndPersistValue('s_prop4',s_vp_getValue('s_eVar4'),0);
	
	s_vp_setAndPersistValue('s_prop5',s_vp_getValue('s_eVar5'),0);

	s_vp_setAndPersistValue('s_prop6',s_vp_getValue('s_eVar6'),0);
	
	s_vp_setAndPersistValue('s_prop18',s_vp_getValue('s_eVar8'),0);
	
	s_vp_setAndPersistValue('s_prop19',s_vp_getValue('s_eVar9'),0);
}
...
/*
 * setValOnce for campaign id
 */
function s_vp_setValOnce(vs) {
	var cn='s_p1_'+vs,vsk=s_c_r(cn),vsv=s_vp_getValue(vs)
    if(vsv){
    	vsv==vsk?s_vpr(vs,''):s_c_w(cn,vsv,0)
    	}
    }

/*

 * Plugin: Set and Persist Value

 *  Param1 - destination variable (s_prop)

 *  Param2 - source variable

 *  Param3 - expiration in days (0 for session)

 */

function s_vp_setAndPersistValue(vs,v,a)

	{var l,e=new Date;if(v){e.setTime(e.getTime()+a*86400000);

	s_c_w('s_p_'+vs,v,a?e:0)};l=s_c_r('s_p_'+vs);s_vpr(vs,l)}	
...
return s_c_r(k)==v}return 0}function s_cet(f,a,et,oe,fb){var r,d=0;if(
...
im,b,e,rs='http'+(s_ssl?'s':'')+'://'+(s_ssl?s_sslfpcsubdomain:s_fpcsubdomain)+'.'+s_fpcdomain+'/b/ss/'+un+'/1/G.9p1/'
...
t);if(s_lnk||s_eo){v=v?v+',pageName,pageURL,referrer,charSet,visitorN'

+'amespace,cookieDomainPeriods,cookieLifetime,currencyCode,purchaseID'

:'';if(v&&!s_pt(v,',',s_isf,t))s_wd[k]='';if(t=='events'&&e)s_wd[k]=

s_fs(s_wd[k],e)}s_wd[m]=0;if(t=='pageURL')q='g';else if(t=='referrer')

q='r';else if(t=='vmk')q='vmt';else if(t=='charSet'){q='ce';if(s_wd[k]&&s_em==2)s_wd[k]='UTF-8'

}else if(t=='visitorNamespace')q='ns';else if(t=='cookieDomainPeriods'

)q='cdp';else if(t=='cookieLifetime')q='cl';else if(t=='currencyCode')

q='cc';else if(t=='channel')q='ch';else if(t=='campaign')q='v0'

else if(s_num(s)){if(b=='prop')q='c'+n;else if(b=='eVar')q='v'+n

else if(b=='hier'){q='h'+n;s_wd[k]=s_fl(s_wd[k],255)}}if(s_wd[k]&&t!=

'linkName'&&t!='linkType')s_qav+='&'+q+'='+s_ape(s_wd[k]);return ''}

function s_hav(){var n,av='vmk,charSet,visitorNamespace,cookieDomainPerio'
...
/* debug code below -- do not alter anything above this line
*/
	var BW_source = location.search.substring(1);
	var BW_match = BW_source.toLowerCase();
	
	if (BW_match.indexOf("debug") != -1) {
	
		document.write('<div style="background:#fff;padding:10px;border:1px solid #f00;font-family:trebuchet ms, arial, helvetica;font-size:10px;font-weight:bold;width:280px;">');
	
		document.write('tracking values:<br\/><br\/>');
	
		var write_pageName = (!s_pageName)?(document.write('s_pageName=none <br \/><br \/>')):(document.write('s_pageName= ' + s_pageName + '<br \/><br \/>'));
	
		var write_prop1 = (!s_prop1)?(document.write('s_prop1=none <br \/><br \/>')):(document.write('s_prop1= ' + s_prop1 + '<br \/><br \/>'));
	
		var write_content_group = (!window.epulse_content_group)?(document.write('window.epulse_content_group=none <br \/><br \/>')):(document.write(window.epulse_content_group + '<br \/><br \/>'));
	
		if (BW_source.indexOf("campaign") != -1) {
	
			var BW_x = unescape(BW_source);
		
			var BW_y = BW_x.replace('debug',"");
			
			if (BW_source.indexOf("&amp;") != -1) {
			
				var BW_z = BW_y.replace(/&amp;/g,"&#160;&#160;");
			...
			document.write(BW_z);
				...
				document.write("campaign tracking=none");
